...
setwd("/home/sbi6dap/Projects/NON-ATHAL/BMR/tophat/")

####### Do one sample only ############
cov<-read.table("/home/sbi6dap/Projects/NON-ATHAL/BMR/tophat/BMR01_hist_exome.txt")
cov_cumul <- 1-cumsum(cov[,5])
plot(cov[2:101, 2], cov_cumul[1:100], type='n', xlab="Depth", ylab="Fraction of capture target bases \u2265 depth", ylim=c(0,1.0), main="Target Region Coverage")
points(cov[2:101, 2], cov_cumul[1:100], type='l', lwd=3)
####### Do one sample only ############

# Get a list of the bedtools output files you'd like to read in
print(files <- list.files(pattern="exome.txt$"))
...
print(labs <- paste(gsub("prefixToTrash-0|\\.pe\\.on\\.pos\\.dedup\\.realigned\\.recalibrated\\.bam\\.cov\\.hist\\.txt\\.all\\.txt", "", files, perl=TRUE), sep=""))
...
plot(cov[[1]][2:101, 2], cov_cumul[[1]][1:100], type='n', xlab="Depth", ylab="Fraction of capture target bases \u2265 depth", ylim=c(0,1.0), main="Target Region Coverage")
...
for (i in 1:length(cov)) points(cov[[i]][2:101, 2], cov_cumul[[i]][1:100], type='l', lwd=3, col=cols[i])
